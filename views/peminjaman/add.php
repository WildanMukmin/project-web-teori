  ...
  <div class="bg-white rounded-xl shadow-xl p-8 w-full max-w-2xl">
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        $tanggal_besok = date('Y-m-d', strtotime('+1 day'));
        ?>
        <div>
          <label class="block font-medium text-gray-700 mb-1">Tanggal Pinjam</label>
          <input type="date" name="tanggal_peminjaman" value="<?= $tanggal_hari_ini ?>" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400" required>
        </div>
        <div>
          <label class="block font-medium text-gray-700 mb-1">Tanggal Pengembalian</label>
          <input type="date" name="tanggal_pengembalian" value="<?= $tanggal_besok ?>" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400">
        </div>
      </div>
